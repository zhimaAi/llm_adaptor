...
func NormalizeImageResponse(ctx context.Context, config provider.Config, providerID provider.ID, hint string, request ImageRequestOptions, response *image.GenerateResponse) error {
	if response == nil || len(response.Data) == 0 {
		return fmt.Errorf("%w: image response contains no data", provider.ErrInvalidRequest)
	}
	format := NormalizeImageFormat(response.OutputFormat)
	for index := range response.Data {
		itemFormat, err := NormalizeImageData(ctx, config, providerID, hint, request, &response.Data[index])
		if err != nil {
			return err
		}
		if format == "" {
...
func NormalizeImageData(ctx context.Context, config provider.Config, providerID provider.ID, hint string, request ImageRequestOptions, data *image.Data) (string, error) {
	...
	if request.ResponseFormat == imageResponseFormatBase64 && data.B64JSON == "" && data.URL != "" {
		payload, downloadedMIMEType, err := downloadImage(ctx, config.HTTPClient, providerID, hint, data.URL)
		if err != nil {
			return "", err
		}
	...
	if data.B64JSON != "" {
		if _, err := base64.StdEncoding.DecodeString(data.B64JSON); err != nil {
			return "", fmt.Errorf("%w: invalid base64 image: %v", provider.ErrInvalidRequest, err)
		}
	}
	...
	if request.ResponseFormat == imageResponseFormatBase64 && data.B64JSON == "" {
		return "", fmt.Errorf("%w: base64 image data is required", provider.ErrInvalidRequest)
	}
	return format, nil
}
...
func ParseImageDataURL(value string) (string, string, error) {
	...
	if comma < 0 {
		return "", "", fmt.Errorf("%w: invalid image data URL", provider.ErrInvalidRequest)
	}
	header, encoded := value[5:comma], value[comma+1:]
	if !strings.Contains(header, ";base64") {
		return "", "", fmt.Errorf("%w: image data URL is not base64 encoded", provider.ErrInvalidRequest)
	}
	mimeType := strings.TrimSpace(strings.SplitN(header, ";", 2)[0])
	if _, err := base64.StdEncoding.DecodeString(encoded); err != nil {
		return "", "", fmt.Errorf("%w: invalid image data URL: %v", provider.ErrInvalidRequest, err)
	}
...
func downloadImage(ctx context.Context, client *http.Client, providerID provider.ID, hint, rawURL string) ([]byte, string, error) {
	...
	if response.StatusCode < http.StatusOK || response.StatusCode >= http.StatusBadRequest {
		return nil, "", &provider.APIError{Provider: providerID, StatusCode: response.StatusCode, Message: response.Status, CredentialHint: hint}
	}
	...
	if len(payload) == 0 {
		return nil, "", fmt.Errorf("%w: downloaded image is empty", provider.ErrInvalidRequest)
	}
